...
void main() {
    vec2 uv = texcoord;
    float xWarpAmount = (isEyeInWater == 1 ? 0.005 : 0) + 0.01 * (1.0 - currentPlayerAir);
    float yWarpAmount = (isEyeInWater == 1 ? 0.005 : 0) + (is_burning ? 0.01 : 0) + 0.01 * (1.0 - currentPlayerAir);
    ...
    if (isEyeInWater == 1) {
        color.rgb = mix(color.rgb, vec3(0.0, 0.0, 1)*(1.0 - currentPlayerAir), 0.5);
    }
    if (is_burning) {
